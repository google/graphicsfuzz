...
public class ExpressionGenerator {
   ...
   * <p>
   * Given the expected value, this method will declare a new zero-initialized variable and a for
   ...
   *
   * For example, given x value we can derive a for loop statement and a remainder from the
   * equation x = (divisor * iterations) + remainder.
   *
   * int _GLF_PRIMITIVE_VAR_INT_7 = 0;
   * for(int i = 0; i < 3; i ++) {      // 3 is the number of iterations obtained by divide the
  ...
  private Expr generateForLoopValue(FactManager factManager,
    ...
    // Having a global fact manager as an input means that the return expression must be
    // available in the global scope. However the new for loop statement generated by this
    // method will be injected into the body of a given function making it available only in
    // the local scope. Hence, if the global scope expression is requested, null will be returned.
    if (factManager.isGlobalManager()) {
      return null;
    }
    ...
    // A random number that will be added to the newly declared variable for each iteration.
    int divisor = generator.nextInt(original + 1);
    // This is to prevent the division by zero error that might occur when a random divisor
    // generated by the random wrapper is zero.
    if (divisor == 0) {
      divisor++;
    }
    ...
    // Value of the number will be added into the variable for each iteration.
    ...
    final Expr increment = new UnaryExpr(new VariableIdentifierExpr("i"), isIncrement ?
        UnOp.POST_INC : UnOp.POST_DEC);
