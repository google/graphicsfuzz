...
public class ExpressionGenerator {
  ...
  // A util method performing the addition of the interface Number.
  ...
  // A util method performing the division of the interface Number.
  private Number divideNumbers(Number firstOperand, Number secondOperand) {
    if (firstOperand instanceof Float || secondOperand instanceof Float) {
      return firstOperand.floatValue() / secondOperand.floatValue();
    } else {
      return firstOperand.intValue() / secondOperand.intValue();
  ...
  private Expr generateAdditionValue(Value value,
    ...
    if (!knownValues.isEmpty() && genSummandsFromKnownValues) {
      return generateSummandFromKnownValues(knownValues, value, factManager, currentFunction,
          stmtToInsertBefore);
    } else {
      return generateSummandFromRandomNumbers(value, factManager, currentFunction,
          stmtToInsertBefore);
    }
  }

  /**
   * This function chooses numbers to be the operands of the addition based on the known facts
   * hold by the fact manager. For example, if we are going to generate an expression that
   * computes 10 and we are holding facts of values 3, 6, 7, and 8. We randomly choose a
   * candidate from these facts and derive an addition expression using the chosen number:
   * - COMPUTE_INT_3 + 7.
   * - COMPUTE_INT_3 + COMPUTE_INT_7.
   * - COMPUTE_INT_6 + (generate further expression that computes 4).
   *
   * @param values          a list of values having the same type as the expectedValue.
   * @param expectedValue   value that an expression generated by this function is computing.
   * @param factManager     manager class holding the value and its associated expression that
   *                        guarantees to compute the given value.
   * @param currentFunction a function into which the new expression will be injected.
   * @return a binary expression that sums two numbers whose result is equal to the expected value.
   */
  private Expr generateSummandFromKnownValues(List<Value> values, Value expectedValue,
                                              FactManager factManager,
                                              FunctionDefinition currentFunction,
                                              Stmt stmtToInsertBefore) {
    assert expectedValue instanceof NumericValue && !values.isEmpty();

    final NumericValue numericValue = (NumericValue) expectedValue;
    final NumericValue candidateValue = (NumericValue) values.get(generator.nextInt(values.size()));
    // To get the second summand, we subtract original value by the chosen candidate.
    final Number right = subtractNumbers(numericValue.getValue().get(),
        candidateValue.getValue().get());
    return new BinaryExpr(
        getExprFromKnownValue(factManager, candidateValue, currentFunction,
            stmtToInsertBefore),
        generateExpr(factManager, currentFunction, stmtToInsertBefore,
            new NumericValue((BasicType) expectedValue.getType(), Optional.of(right))),
        BinOp.ADD);
  }

  /**
   * A function that generates an expression performing the addition of two numbers that
   * guarantees to produce the same result as the given value. The operands of the addition are
   * chosen randomly.
   *
   * @param value              value that an expression generated by this function is computing.
   * @param factManager        manager class holding the value and its associated expression that
   *                           guarantees to compute the given value.
   * @param currentFunction    a function into which the new expression will be injected.
   * @param stmtToInsertBefore statement in the body of the given function where the generated
   ...
   * @return a binary expression that sums two numbers whose result is equal to the expected value.
   */
  private Expr generateSummandFromRandomNumbers(Value value, FactManager factManager,
                                                FunctionDefinition currentFunction,
                                                Stmt stmtToInsertBefore) {
    ...
    final Number a = numericValue.getValue().get();
    final Number b = Math.max(1, generator.nextInt(10));
    final Number left = divideNumbers(a, b);
    final Number right = subtractNumbers(a, left);
  ...
  /**
   * Given a value, this function searches for the existing facts and return its associated
   * expression that computes the given value.
   *
   * @param factManager        manager class holding the value and its associated expression that
   *                           guarantees to compute the given value.
   * @param value              the value which the given fact manager is holding.
   * @param currentFunction    a function into which the new expression will be injected.
   * @param stmtToInsertBefore statement in the body of the given function where the generated
   *                           expression will be inserted before.
   * @return A return expression could be either, in the case of function fact, function call or
   *     variable identifier in the case of variable fact.
   */
  private Expr getExprFromKnownValue(FactManager factManager, Value value,
                                     FunctionDefinition currentFunction,
                                     Stmt stmtToInsertBefore) {
    Expr expr;
    while (true) {
      if (generator.nextBoolean()) {
        expr = generateKnownFunctionFact(factManager, value, currentFunction,
            stmtToInsertBefore);
      } else {
        expr = generateKnownVariableFact(value, factManager);
      }
      if (expr != null) {
        break;
      }
