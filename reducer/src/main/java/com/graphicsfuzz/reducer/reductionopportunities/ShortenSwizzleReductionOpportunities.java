 ...
 */
public class ShortenSwizzleReductionOpportunities
    extends ReductionOpportunitiesBase<ShortenSwizzleReductionOpportunity> {

  private final Typer typer;
  private final TranslationUnit tu;

  static List<ShortenSwizzleReductionOpportunity> findOpportunities(
      ShaderJob shaderJob,
  ...
  private static List<ShortenSwizzleReductionOpportunity> findOpportunitiesForShader(
      TranslationUnit tu,
      ReducerContext context) {
    ShortenSwizzleReductionOpportunities finder =
        new ShortenSwizzleReductionOpportunities(tu, context);
    finder.visit(tu);
    return finder.getOpportunities();
  }

  private ShortenSwizzleReductionOpportunities(TranslationUnit tu,
                                                ReducerContext context) {
    super(tu, context);
  ...
  public void visitMemberLookupExpr(MemberLookupExpr memberLookupExpr) {
    ...
    final int minimumLength = tu.getShadingLanguageVersion().supportedScalarSwizzle() ? 1 : 2;
    for (int newLength = Math.max(minimumLength,
        RemoveSwizzleReductionOpportunity.getLargestSwizzleComponent(parent) + 1);
         newLength < memberLookupExpr.getMember().length(); newLength++) {
      addOpportunity(new ShortenSwizzleReductionOpportunity((MemberLookupExpr) parent,
          memberLookupExpr, newLength,
          getVistitationDepth()));
    }
  }
}
