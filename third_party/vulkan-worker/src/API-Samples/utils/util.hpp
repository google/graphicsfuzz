...
#define APP_NAME_STR_LEN 80
    ...
    std::vector<struct uniform_data> uniform_datas;
    ...
    std::vector<struct uniform_spec> sample_uniforms;
...
// Hugues: be able to force termination.
void requestAppFinish();
...
#define VKLOGFILE "/sdcard/graphicsfuzz/log.txt"

#define vklog(fmt, ...) do {\
        FILE *_log_f = fopen(VKLOGFILE, "a");\
        if (_log_f == NULL) { LOGE("Cannot open log file"); exit(1); }\
        fprintf(_log_f, fmt, ## __VA_ARGS__);\
        fclose(_log_f);\
    } while (0)

VkResult vkCallLog(VkResult res, const char *expr, const char *file, int line);
#define VKCALL(EXPR) vkCallLog((EXPR), #EXPR, __FILE__, __LINE__)
