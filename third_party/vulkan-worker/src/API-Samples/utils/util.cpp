/*
 * Vulkan Samples - edited for GraphicsFuzz vulkan worker
 *
...
void write_ppm(struct sample_info &info, const char *basename) {
    ...
    // Hugues: make sure to flush properly
    file.flush();
...
void requestAppFinish() {
    ANativeActivity_finish(Android_application->activity);
}
...
const char *vkGetResultString(VkResult res) {
    switch (res) {
...
VkResult vkCallLog(VkResult res, const char *expr, const char *file, int line) {
    FILE *f = fopen(VKLOGFILE, "a");
    if (f == NULL) {
        LOGE("Failed to open log file");
        exit(1);
    }
    (void) file;
    fprintf(f, "L%4.4d: %s: %s\n", line, expr, vkGetResultString(res));
    fflush(f);
    fclose(f);
    return res;
}
