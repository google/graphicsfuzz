...
layout(set = 0, binding = 0, std140) buffer buf {
    VertexData vertices[];
};
...
void main() {
  ...
  vertices[0].position = 0.2 * rotate * vertices[0].position;
  vertices[1].position = 0.2 * rotate * vertices[1].position;
  vertices[2].position = 0.2 * rotate * vertices[2].position;
...
void populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& vkDebugMessengerCreateInfo) {
  vkDebugMessengerCreateInfo = {};
  vkDebugMessengerCreateInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
  vkDebugMessengerCreateInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
  vkDebugMessengerCreateInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT;
...
bool isDeviceSuitable(VkPhysicalDevice device) {
  auto graphicsQueueIndex = findGraphicsQueueFamilyIndex(device);
  auto computeQueueIndex = findComputeQueueFamilyIndex(device);
  bool extensionsSupported = checkDeviceExtensionSupport(device);
  return graphicsQueueIndex.has_value() && computeQueueIndex.has_value() && extensionsSupported;
}
...
VkShaderModule createShaderModule(const std::vector<uint32_t>& code, VkDevice device) {
  ...
  if (vkCreateShaderModule(device, &vkShaderModuleCreateInfo, nullptr, &shaderModule) != VK_SUCCESS) {
...
const std::vector<Vertex> vertices = {
        {{0.0f, -0.5f}, {0.0, 0.0}, {1.0f, 0.0f, 0.0f}, {0.0}},
        {{0.5f, 0.5f}, {0.0, 0.0}, {0.0f, 1.0f, 0.0f}, {0.0}},
        {{-0.5f, 0.5f}, {0.0, 0.0}, {0.0f, 0.0f, 1.0f}, {0.0}}
};
...
int main() {
  ...
  try {
    ...
    if (enableValidationLayers) {
      ...
      if (CreateDebugUtilsMessengerEXT(instance, &vkDebugUtilsMessengerCreateInfo, nullptr, &debugMessenger) != VK_SUCCESS) {
    ...
    // Find queue families
    auto graphicsQueueFamilyIndex = findGraphicsQueueFamilyIndex(
            physicalDevice).value();
    auto computeQueueFamilyIndex = findComputeQueueFamilyIndex(
            physicalDevice).value();
    std::set<uint32_t> uniqueQueueFamilyIndices;
    uniqueQueueFamilyIndices.insert(graphicsQueueFamilyIndex);
    uniqueQueueFamilyIndices.insert(computeQueueFamilyIndex);
    ...
    if (vkCreateDevice(physicalDevice, &vkDeviceCreateInfo, nullptr, &device) != VK_SUCCESS) {
    ...
    if (vkCreateImage(device, &vkImageCreateInfo, nullptr, &offScreenImage) != VK_SUCCESS) {
      throw std::runtime_error("Failed to create image.");
    }
    VkMemoryRequirements imageMemoryRequirements;
    vkGetImageMemoryRequirements(device, offScreenImage, &imageMemoryRequirements);
    {
      ...
      vkMemoryAllocInfo.memoryTypeIndex = findMemoryType(
    ...
    if (vkBindImageMemory(device, offScreenImage, offScreenImageMemory, 0) != VK_SUCCESS) {
    ...
    if (vkCreateImageView(device, &vkImageViewCreateInfo, nullptr, &offScreenImageView) != VK_SUCCESS) {
    ...
    vertexShader.setEnvInput(glslang::EShSourceGlsl, EShLangVertex, glslang::EShClientVulkan, glslang::EShTargetVulkan_1_0);
    vertexShader.setEnvClient(glslang::EShClientVulkan, glslang::EShTargetVulkan_1_0);
    ...
    glslang::GlslangToSpv(*vertexShaderProgram.getIntermediate(EShLangVertex), vertexShaderBinary);
    glslang::TShader fragmentShader(EShLangFragment);
    fragmentShader.setStrings(&fragmentShaderText, 1);
    fragmentShader.setEnvInput(glslang::EShSourceGlsl, EShLangFragment, glslang::EShClientVulkan, glslang::EShTargetVulkan_1_0);
    fragmentShader.setEnvClient(glslang::EShClientVulkan, glslang::EShTargetVulkan_1_0);
    ...
    glslang::GlslangToSpv(*fragmentShaderProgram.getIntermediate(EShLangFragment), fragmentShaderBinary);
    glslang::TShader computeShader(EShLangCompute);
    computeShader.setStrings(&computeShaderText, 1);
    computeShader.setEnvInput(glslang::EShSourceGlsl, EShLangCompute, glslang::EShClientVulkan, glslang::EShTargetVulkan_1_0);
    computeShader.setEnvClient(glslang::EShClientVulkan, glslang::EShTargetVulkan_1_0);
    computeShader.setEnvTarget(glslang::EShTargetSpv, glslang::EShTargetSpv_1_0);
    if (!computeShader.parse(&DefaultTBuiltInResource, 100, false, EShMsgDefault)) {
      throw std::runtime_error("Error compiling compute shader to SPIR-V.");
    }
    glslang::TProgram computeShaderProgram;
    computeShaderProgram.addShader(&computeShader);
    if (!computeShaderProgram.link(EShMsgDefault)) {
      throw std::runtime_error("Error linking compute shader program.");
    }
    std::vector<uint32_t> computeShaderBinary;
    glslang::GlslangToSpv(*computeShaderProgram.getIntermediate(EShLangCompute), computeShaderBinary);
    ShFinalize();
    ...
    // Create vertex buffer that can also be manipulated by compute shader.
    ...
    if (vkCreateBuffer(device, &vkBufferCreateInfo, nullptr, &vertexBuffer) != VK_SUCCESS) {
    ...
    vkGetBufferMemoryRequirements(device, vertexBuffer, &vertexBufferMemoryRequirements);
    {
      ...
      vkMemoryAllocateInfo.memoryTypeIndex = findMemoryType(
              ...
              physicalDevice);
      if (vkAllocateMemory(device, &vkMemoryAllocateInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS) {
    ...
    {
      void *data;
      vkMapMemory(device, vertexBufferMemory, 0, vkBufferCreateInfo.size, 0, &data);
      memcpy(data, vertices.data(), static_cast<uint32_t>(vkBufferCreateInfo.size));
    ...
    {
      VkShaderModule compShaderModule = createShaderModule(computeShaderBinary,
                                                           device);
    ...
    // Create and populate compute command buffer
    {
      VkCommandBufferAllocateInfo vkCommandBufferAllocateInfo = {};
      vkCommandBufferAllocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
      vkCommandBufferAllocateInfo.commandPool = computeCommandPool;
      ...
      if (vkBeginCommandBuffer(computeCommandBuffer, &beginInfo) !=
          VK_SUCCESS) {
    ...
    // Create and populate graphics command buffer
    {
      VkCommandBufferAllocateInfo vkCommandBufferAllocateInfo = {};
      vkCommandBufferAllocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
      vkCommandBufferAllocateInfo.commandPool = graphicsCommandPool;
      vkCommandBufferAllocateInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
      vkCommandBufferAllocateInfo.commandBufferCount = 1;
      if (vkAllocateCommandBuffers(device, &vkCommandBufferAllocateInfo,
                                   &graphicsCommandBuffer) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate graphics command buffers.");
      }
      VkCommandBufferBeginInfo beginInfo = {};
      beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
      if (vkBeginCommandBuffer(graphicsCommandBuffer, &beginInfo) !=
          VK_SUCCESS) {
        throw std::runtime_error("Failed to begin recording command buffer.");
      }

      // Barrier to ensure that results from the compute shader are available.
      ...
      computeFinishBarrier.dstAccessMask = VK_ACCESS_INDEX_READ_BIT;
      ...
      vkCmdBeginRenderPass(graphicsCommandBuffer, &renderPassInfo,
                           VK_SUBPASS_CONTENTS_INLINE);
      vkCmdBindPipeline(graphicsCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                        graphicsPipeline);
      VkBuffer vertexBuffers[] = {vertexBuffer};
      VkDeviceSize offsets[] = {0};
      vkCmdBindVertexBuffers(graphicsCommandBuffer, 0, 1, vertexBuffers,
                             offsets);
    ...
    // Submit the compute work
    {
      ...
      if (vkQueueSubmit(computeQueue, 1, &submitInfo, VK_NULL_HANDLE) != VK_SUCCESS) {
        throw std::runtime_error("Failed to submit compute workload.");
      }
    ...
    createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, stagingBuffer, stagingBufferMemory, device, physicalDevice);

    {
      VkCommandBufferAllocateInfo tempCommandBufferAllocateInfo = {};
      tempCommandBufferAllocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
      tempCommandBufferAllocateInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
      tempCommandBufferAllocateInfo.commandPool = graphicsCommandPool;
      tempCommandBufferAllocateInfo.commandBufferCount = 1;
      VkCommandBuffer tempCommandBuffer;
      vkAllocateCommandBuffers(device, &tempCommandBufferAllocateInfo, &tempCommandBuffer);
      ...
      renderFinishBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
      ...
      vkCmdPipelineBarrier(tempCommandBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &renderFinishBarrier);
      ...
      vkCmdCopyImageToBuffer(tempCommandBuffer, offScreenImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, stagingBuffer, 1, &region);
      ...
      vkCmdPipelineBarrier(tempCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 0,
                           nullptr, 1, &copyFinishBarrier, 0, nullptr);
      ...
      vkFreeCommandBuffers(device, graphicsCommandPool, 1, &tempCommandBuffer);
    }
    ...
    stbi_write_png("out.png", WIDTH, HEIGHT, NUM_CHANNELS, pixels, WIDTH * NUM_CHANNELS);
