...
int main() {
  ...
  try {
    ...
    // Find queue families
    std::optional<uint32_t> graphicsQueueIndex = findGraphicsQueueFamilyIndex(
            physicalDevice);
    ...
    VkShaderModule vertShaderModule = createShaderModule(vertexShaderBinary, device);
    VkShaderModule fragShaderModule = createShaderModule(fragmentShaderBinary, device);
    ...
    if (vkAllocateCommandBuffers(device, &vkCommandBufferAllocateInfo, &commandBuffer) != VK_SUCCESS) {
