...
int main() {
  ...
  try {
    ...
    vkMemoryAllocInfo.allocationSize = memRequirements.size;
    vkMemoryAllocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, physicalDevice);
    if (vkAllocateMemory(device, &vkMemoryAllocInfo, nullptr, &offScreenImageMemory) != VK_SUCCESS) {
