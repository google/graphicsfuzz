...
int main() {
  ...
  try {
    ...
    // Compute the minimum alignment of uniform buffer dynamic offset.
    VkPhysicalDeviceProperties device_properties;
    vkGetPhysicalDeviceProperties(physicalDevice, &device_properties);

    VkDeviceSize uniform_buffer_object_alignment = sizeof(glm::mat4);
    VkDeviceSize min_alignment =
        device_properties.limits.minUniformBufferOffsetAlignment;
    if (min_alignment > 0) {
      uniform_buffer_object_alignment =
          std::max(uniform_buffer_object_alignment, min_alignment);
    }
    ...
    // Create uniform buffer
    {
      ...
      create_info.flags = 0;
      create_info.size =
          uniform_buffer_object_alignment * 2;  // Two 4x4 matrices.
      create_info.usage =
    ...
    // Allocate & update descriptor set
    {
      ...
      descriptor_buffer_info.range = uniform_buffer_object_alignment;
    ...
    // Submit command buffer that updates the the uniform buffer.
    {
      ...
      vkCmdUpdateBuffer(commandBuffer, uniform_buffer, 0, sizeof(glm::mat4),
                        &identity);
      vkCmdUpdateBuffer(commandBuffer, uniform_buffer,
                        uniform_buffer_object_alignment, sizeof(glm::mat4),
                        &rotate);
    ...
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      graphicsPipeline);
    uint32_t dynamic_offsets = uniform_buffer_object_alignment;
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            pipelineLayout, 0, 1, &descriptor_set, 1,
                            &dynamic_offsets);
